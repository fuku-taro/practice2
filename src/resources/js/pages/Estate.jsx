...
export default function Estate() {
  ...
  const rows = [
    ...
    createData('所在地', `${data[0].location1}${data[0].location2}${data[0].address}`),
    createData('沿線・駅・交通', `${data[0].nearest_line}${data[0].station}駅 徒歩${data[0].walk_time}分`),
    createData('間取り', `${data[0].floor_plan}`),
    createData('間取り内訳', `${data[0].floor_plan_kind}`),
    createData('サービスルーム数', ``),
    createData('土地面積', `${data[0].measurement_method}${data[0].land_area}㎡`),
  ...
  const rows2 = [
    ...
    createData('築年月', `${data[0].build_year}年${data[0].build_mounth}月`),
    ...
    createData('建・容率', `${data[0].building_ratio}${data[0].floor_area_ratio}`),
    createData('地勢', ``),
    createData('地域地区', ``),
    createData('傾斜地面積', ``),
    createData('接道状況詳細',``),
